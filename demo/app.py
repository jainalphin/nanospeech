...
def get_prompt_list(prompt_dir: str):
    ...
    for wav_path in wav_paths:
        ...
        prompt_lookup[voice_name] = VoicePrompt(wav_path=wav_path, text=text)
...
def create_demo(prompt_list: dict[str, VoicePrompt], model: "Nanospeech", header: str):
    def update_prompt(voice_name: str):
        return (prompt_list[voice_name].wav_path, prompt_list[voice_name].text)
    ...
    def _generate(
        ...
        if is_single_generation:
            ...
            if hasattr(wave, "numpy"):
                wave = convert_audio_float32_to_int16(wave.numpy().squeeze(0))
            else:
                wave = convert_audio_float32_to_int16(np.array(wave))
        else:
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column():
                input_text = gr.Textbox(label="Text", value="The quick brown fox jumped over the lazy dog.")

                with gr.Group():
                    voice_dropdown = gr.Dropdown(choices=list(prompt_list.keys()), value="orion", interactive=True, label="Voice")
                    prompt_audio = gr.Audio(label="Audio", value=prompt_list[voice_dropdown.value].wav_path)
                    prompt_text = gr.Textbox(label="Reference Text", value=prompt_list[voice_dropdown.value].text, interactive=False)

                with gr.Accordion("Additional Settings", open=False):
                    speed = gr.Slider(label="Speed", value=1.0, minimum=0.1, maximum=2.0, step=0.1)
                    nfe_steps = gr.Slider(
                        label="NFE Steps - more steps will have higher quality, but slower generation times",
                        value=8,
                        minimum=4,
                        maximum=128,
                        step=1,
                    ...
                    sway_sampling_coef = gr.Slider(label="Sway Sampling Coefficient", value=-1.0, minimum=-1.0, maximum=1.0, step=0.1)
        ...
        generate_button.click(
            fn=_generate,
            inputs=[prompt_audio, prompt_text, input_text, nfe_steps, method, cfg_strength, sway_sampling_coef, speed],
